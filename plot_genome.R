...
#'
#' @return A vector containing IR region information:
#' \itemize{
...
#' }
get_ir <- function(gbfile, local.file = FALSE){
  ...
  my_env$genome <- NULL
  my_env$l <- NULL
  my_env$gene_table <- NULL # can't delete for some reason TODO
  my_env$ir <- NULL
  if (local.file){
    tryCatch({
  ...
  # gene_len <- tail(ir_table$end, n=1)
  ...
  if(nrow(IRB) == 1) {
    ...
    # if(IR_len != as.numeric(IRB$length)){
    #   print(paste('Lengths don\'t match. IRA length:', IR_len, 'vs. IRB length:',
    #               as.numeric(IRB$length)))
    # }
  } else {
    IRB_start <- IRB$start[2] + 1
    # if(IR_len != as.numeric(IRB$length[2])){
    #   print(paste('Lengths don\'t match. IRA length:', IR_len, 'vs. IRB length:',
    #               as.numeric(IRB$length[2])))
    # }
  }

  print(c(IRA_start, IRB_start, IR_len, my_env$l))
  return(c(IRA_start, IRB_start, IR_len, my_env$l))
}
