# TODO: make program work with ir_table dataframe instead of transforming it into a vector
...
#' @param l A number. Indicates the genome length.
#'
#' @return A named list with:
#' \itemize{
#'   \item ir_table: A vector containing IR region information:
#'   \itemize{
...
#'   }
#'   \item indel_table: a dataframe with the mismatches found with these columns:
#'   \itemize{
#'      \item mismatch_type: type of the mismatch.
#'      \item position: where in the genome it is.
#'      \item string: letter changed.
#'      \item col: color.
#'   }
#' }
#' @import Biostrings
#' @export
get_ir <- function(genome){
  ir <- NULL
  tryCatch({
...
#' @param genome A DNAString object. It is a class defined in \code{Biostrings}
#' package. It contains the whole genome sequence of a specie.
#' @param seed.size Seed to set the starting point.
#'
#' @return A named list with:
#' \itemize{
#'   \item ir_table: A dataframe containing IR region information.
#'   \item indel_table: a dataframe with the mismatches found with these columns:
#'   \itemize{
#'      \item mismatch_type: type of the mismatch.
#'      \item position: where in the genome it is.
    ...
    if ((tick - ira_s) == 0){
      ir_table <- data.frame(chr = rep("chr1", 4),
                             ...
                             text = c(paste("IRA:", ira_len),
                                      ...
                                      paste("IRB:", irb_len),
                                      paste("LSC:", lsc_len)),
                             length = c(ira_len, ssc_len, irb_len, lsc_len),
                             stringsAsFactors = FALSE)
    } else {
      ir_table <- data.frame(chr = rep ("chr1", 5),
...
map_genome <- function(genome, seed_starts, seed.size = 1000,
  ...
  # Break the whole genome into small pieces (seeds) with size [seed.size]bp
